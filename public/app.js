...
const formatMYT=iso=>{if(!iso)return 'Last update —';const d=new Date(iso);if(Number.isNaN(d.getTime()))return 'Last update —';return 'Last update '+new Intl.DateTimeFormat('en-MY',{timeZone:'Asia/Kuala_Lumpur',day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit',hour12:true}).format(d)+' MYT'};
function applyTheme(theme){const t=theme==='light'?'light':'dark';document.documentElement.dataset.theme=t;document.documentElement.style.colorScheme=t;localStorage.setItem(THEME_KEY,t);const b=$('#themeToggle');if(b){b.textContent=t==='dark'?'☀':'☾';b.title=t==='dark'?'Switch to day mode':'Switch to night mode';b.setAttribute('aria-label',b.title)}const meta=document.querySelector('meta[name=theme-color]');if(meta)meta.content=t==='dark'?'#07111e':'#f4f7fb'}
function initTheme(){const saved=localStorage.getItem(THEME_KEY);const preferred=window.matchMedia&&window.matchMedia('(prefers-color-scheme: light)').matches?'light':'dark';applyTheme(saved||preferred)}
