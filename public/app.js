...
const statusPriority={SELL:0,NEAR_SELL:1,BUY_SIGNAL:2,ENTRY_CONFIRMED:3};

function sortSignalEvents(events,allEvents){
  const deltaPriority={NEW:0,CHANGED:1,'':2};
  return [...events].sort((a,b)=>{
    const status=(statusPriority[a.event_type]??99)-(statusPriority[b.event_type]??99);
...
function strategyGroup(strategy,filtered,allEvents){
  const visible=sortSignalEvents(filtered.filter(x=>x.strategy===strategy),allEvents);
...
function openStopDistance(x){
  const latest=Number(x.latest_close),stop=Number(x.atr_stop);
  return Number.isFinite(latest)&&latest>0&&Number.isFinite(stop)&&stop>0?(latest/stop-1)*100:null
}

function openUnrealized(x){
  const latest=Number(x.latest_close),entry=Number(x.entry_price);
  return Number.isFinite(latest)&&latest>0&&Number.isFinite(entry)&&entry>0?(latest/entry-1)*100:null
}
...
function sortOpenPositions(a){
  const pri={NEAR_SELL:0,BUY_PENDING:1,OPEN:2};
  return [...a].sort((x,y)=>{
    const p=(pri[x.status]??9)-(pri[y.status]??9);
    if(p)return p;
    if(x.status==='BUY_PENDING'&&y.status==='BUY_PENDING'){
      const d=String(y.signal_date||'').localeCompare(String(x.signal_date||''));
      if(d)return d
    }else{
      const xd=openStopDistance(x),yd=openStopDistance(y);
      const xv=Number.isFinite(xd)?xd:9999,yv=Number.isFinite(yd)?yd:9999;
...
function openSummary(base){
  const active=base.length;
  const near=base.filter(x=>x.status==='NEAR_SELL').length;
  const pending=base.filter(x=>x.status==='BUY_PENDING').length;
  const opened=base.filter(x=>x.status==='OPEN').length;
  const risk=base
    .filter(x=>x.status!=='BUY_PENDING'&&Number.isFinite(openStopDistance(x)))
    .sort((a,b)=>openStopDistance(a)-openStopDistance(b))
    .slice(0,3);

  const counters=`<div class="stats" style="margin:3px 0 12px">
    <div class="stat"><b>${active}</b><span>Active</span></div>
    <div class="stat"><b>${near}</b><span>Near Sell</span></div>
    <div class="stat"><b>${pending}</b><span>Buy Pending</span></div>
    <div class="stat"><b>${opened}</b><span>Open</span></div>
  </div>`;

  const riskCard=risk.length?`<div class="card" style="margin-bottom:12px">
    <div class="row"><div class="ticker">⚠ Closest to ATR Stop</div><div class="name">Risk first</div></div>
    <div class="meta">
      ${risk.map(x=>`<div class="kv"><span>${esc(x.symbol.replace('.KL',''))}</span><b>${openPct(openStopDistance(x))}</b></div>`).join('')}
    </div>
  </div>`:'';

  return counters+riskCard
}

function openPositionCard(x){
  const statusText=String(x.status||'').replaceAll('_',' ');
  const statusClass=x.status==='NEAR_SELL'?'NEAR_SELL':x.status==='BUY_PENDING'?'ENTRY_CONFIRMED':'ENTRY_CONFIRMED';

  if(x.status==='BUY_PENDING'){
    return `<div class="card">
      <div class="top">
        <div><div class="ticker">${esc(x.symbol.replace('.KL',''))}</div><div class="name">${esc(x.name)} · ${esc(labels[x.strategy]||x.strategy)}</div></div>
        <div class="badge ${statusClass}">${esc(statusText)}</div>
      </div>
      <div class="meta">
        <div class="kv"><span>Signal close</span><b>${money(x.latest_close)}</b></div>
        <div class="kv"><span>Planned entry</span><b>NEXT OPEN</b></div>
        <div class="kv"><span>ATR stop</span><b>PENDING</b></div>
        <div class="kv"><span>Signal date</span><b>${esc(x.signal_date||'—')}</b></div>
        <div class="kv"><span>Entry date</span><b>Pending</b></div>
        <div class="kv"><span>Held</span><b>0d</b></div>
      </div>
    </div>`
  }

  const dist=openStopDistance(x);
  const unreal=openUnrealized(x);
  return `<div class="card">
    <div class="top">
      <div><div class="ticker">${esc(x.symbol.replace('.KL',''))}</div><div class="name">${esc(x.name)} · ${esc(labels[x.strategy]||x.strategy)}</div></div>
      <div class="badge ${statusClass}">${esc(statusText)}</div>
    ...
    <div class="meta">
      <div class="kv"><span>Entry</span><b>${money(x.entry_price)}</b></div>
      <div class="kv"><span>Latest</span><b>${money(x.latest_close)}</b></div>
      <div class="kv"><span>ATR stop</span><b>${atrMoney(x.atr_stop)}</b></div>
      <div class="kv"><span>Stop distance</span><b>${openPct(dist)}</b></div>
      <div class="kv"><span>Unrealised</span><b>${openPct(unreal)}</b></div>
      <div class="kv"><span>Held</span><b>${x.hold_days||0}d</b></div>
    </div>
    <div class="msg">Signal ${esc(x.signal_date||'—')} · Entry ${esc(x.entry_date||'—')}</div>
  </div>`
}

function renderOpen(){
  const base=state.positions.filter(x=>state.strategyFilter==='ALL'||x.strategy===state.strategyFilter);
  let a=sortOpenPositions(base);
  const q=($('#openSearch')?.value||'').toLowerCase();
  if(q)a=a.filter(x=>x.symbol.toLowerCase().includes(q)||x.name.toLowerCase().includes(q));
  const summary=openSummary(base);
  const cards=a.length?a.map(openPositionCard).join(''):'<div class="empty">No matching active strategy states.</div>';
  $('#openList').innerHTML=summary+cards
}
